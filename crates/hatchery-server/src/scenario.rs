...
//! in the bestand so it shows up in the graph. Payloads are salted per run so
//! repeated runs don't collide via dedup.
...
pub async fn run(
    ...
    let (result, areas) = state
        .read(move |k| match id.as_str() {
            "dedup" => scn_dedup(k, s),
            "type" => scn_type(k, s),
            "traversal" => scn_traversal(k, s),
            "supersession" => scn_supersession(k, s),
            "gate" => scn_gate(k, s),
            "provenance" => scn_provenance(k, s),
            "anchor" => scn_anchor(k, s),
            "atomicity" => scn_atomicity(k, s),
            "federation" => scn_federation(k, s),
...
fn scn_dedup(k: &Kernel, salt: u128) -> ScnOut {
    let d = Datum::leaf(format!("dedup-demo#{salt}").into_bytes());
...
fn scn_type(k: &Kernel, salt: u128) -> ScnOut {
    let tm = k.append(&vocab::hatchery_type_marker())?;
    let tname = k.append(&Datum::leaf(format!("Person#{salt}").into_bytes()))?;
    let tctx = Datum::node([tm, tname]).ok_or(KernelError::Inconsistent)?;
    let tctx_id = k.append(&tctx)?;
    let content = k.append(&Datum::leaf(format!("Alice#{salt}").into_bytes()))?;
...
fn scn_traversal(k: &Kernel, salt: u128) -> ScnOut {
    let mut prev = k.append(&Datum::leaf(format!("chain-0#{salt}").into_bytes()))?;
...
fn scn_supersession(k: &Kernel, salt: u128) -> ScnOut {
    k.append(&Datum::supersession_marker())?;
    let v1 = k.append(&Datum::leaf(format!("doc-v1#{salt}").into_bytes()))?;
    let sup = k.append(&Datum::supersedes(v1))?;
    let v2c = k.append(&Datum::leaf(format!("doc-v2#{salt}").into_bytes()))?;
...
fn scn_gate(k: &Kernel, salt: u128) -> ScnOut {
    let area = k.append(&Datum::leaf(format!("area/secret#{salt}").into_bytes()))?;
    k.append(&Datum::area_membership_marker())?;
    let am = k.append(&Datum::area_membership(area))?;
    let content = k.append(&Datum::leaf(format!("secret-doc#{salt}").into_bytes()))?;
    let restricted = Datum::node([content, am]).ok_or(KernelError::Inconsistent)?;
    let restricted_id = k.append(&restricted)?;
    let subject = k.append(&Datum::leaf(format!("subject/alice#{salt}").into_bytes()))?;
...
fn scn_provenance(k: &Kernel, salt: u128) -> ScnOut {
    let i1 = k.append(&Datum::leaf(format!("input-1#{salt}").into_bytes()))?;
    let i2 = k.append(&Datum::leaf(format!("input-2#{salt}").into_bytes()))?;
    k.append(&Datum::origin_marker())?;
    k.append(&Datum::origin(i1))?;
    k.append(&Datum::origin(i2))?;
    let payload = k.append(&Datum::leaf(format!("result#{salt}").into_bytes()))?;
...
fn scn_atomicity(k: &Kernel, salt: u128) -> ScnOut {
    let c1 = Datum::leaf(format!("atom-c1#{salt}").into_bytes());
    let c2 = Datum::leaf(format!("atom-c2#{salt}").into_bytes());
    ...
    let passed = before_hidden && after_visible;
    Ok((
        json!({
            "id":"atomicity","axiom":"§13","title":"Aktiv-Marker / Atomarität","passed":passed,
            "detail":"Konstituenten sind unsichtbar bis der Aktiv-Marker sie GEMEINSAM freigibt — Atomarität ohne Transaktions-Maschinerie.",
...
fn scn_federation(k: &Kernel, salt: u128) -> ScnOut {
    let tmp = std::env::temp_dir().join(format!("hatchery-fed-{salt}"));
    std::fs::create_dir_all(&tmp).map_err(|_| KernelError::Io)?;
    let foreign = LakearchKernel::open(&tmp)?;
    let shared = Datum::leaf(format!("shared#{salt}").into_bytes());
    let foreign_only = Datum::leaf(format!("foreign-only#{salt}").into_bytes());
    ...
    let passed = dedup_count >= 1 && new_count >= 1;
    Ok((
        json!({
